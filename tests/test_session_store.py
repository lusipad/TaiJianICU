...
def test_session_store_revival_artifact_paths(tmp_path: Path) -> None:
    ...
    assert store.director_constraints_path("demo") == tmp_path / "demo" / "director_constraints.json"
    assert store.revival_diagnosis_path("demo") == tmp_path / "demo" / "revival_diagnosis.json"
    assert store.trust_report_path("demo") == tmp_path / "demo" / "trust_report.json"
