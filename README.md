...
TaiJianICU 是一个面向中文长篇创作者的 Revival 可信评审工作台。它不是简单补一段文字，而是把原稿拆成可追踪的创作上下文：世界观、人物状态、伏笔、章节推进、导演内部化约束、盲测和可信报告，帮助你判断下一章是否真的像原作者。
...
- CLI：开发者可以直接用 `taijianicu run`、`inspect`、`benchmark`、`benchmark-multi` 跑兼容续写和验证链路。
...
桌面版仍然需要联网访问你配置的模型 API。用户上传的文本、会话、索引、导演约束、可信报告和输出会写在应用旁边的 `data/` 目录。本轮只做清晰说明，暂不提供一键删除本地数据控件。macOS 当前未做签名和公证，首次打开可能需要在系统安全设置里允许运行。

## Studio 工作流

Studio 的主线已经收束为 Revival 工作流，旧普通续写入口保留 CLI/API 兼容但不作为 Studio 第一入口展示：

1. `开始任务`：导入原稿并发起 Revival 原著分析。
2. `工作台`：查看当前任务可信状态、下一步动作和最近结果。
3. `导演计划`：查看作者声纹/人物/伏笔，选择人物走向，编辑“现代意图 → 作品动作”的内部化约束。
4. `章节队列`：查看提纲候选、正文候选和章节产物。
5. `单章评审`：查看按风险排序的可信报告、复活诊断、盲测、失败证据，保存修订提示草稿，并可按草稿发起再生成；重生成后会保留上一版可信报告用于前后对比。
6. `资料库`：只读查看作者声纹、人物状态、伏笔队列、世界约束和产物。
7. `设置`：配置 API Endpoint、Key、模型路由和连接测试，查看数据流说明。
...
- TaiJianICU 仍处于快速实验阶段，当前成功标准是 Revival 可信评审，不应把输出直接当成最终稿。
- 旧 V2 世界/规划/参考能力是辅助层，不是当前成功标准。
